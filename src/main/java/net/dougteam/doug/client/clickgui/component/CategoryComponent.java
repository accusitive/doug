...
public class CategoryComponent extends Component {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY) {
        ...
        if (!this.open) {
            RenderUtils.drawBorderedRect(matrixStack, this.x, this.y, this.x + this.width, this.y + this.height(), 1,
                    Client.panelColor(), hovered(mouseX, mouseY) ? Client.panelSelectedColor() : -1);
        } else {
            RenderUtils.drawBorderedRectHoriTop(matrixStack, this.x, this.y, this.x + this.width,
                    this.y + this.height(), 1, Client.panelColor(),
                    hovered(mouseX, mouseY) ? Client.panelSelectedColor() : -1);

        }
        mc.inGameHud.getFontRenderer().drawWithShadow(matrixStack, s, this.x + (this.width - w) / 2, this.y + 3,
                this.dragging ? Client.mainColor() : -1);
